...
try {
    ...
    Write-Host "`n3/5 Deny an over-scoped deploy"
    ...
    Write-Host "`n4/5 Run the adversarial conformance suite"
    python -m permitmesh conformance examples\conformance-suite.json
    if ($LASTEXITCODE -ne 0) { throw "conformance suite failed" }

    Write-Host "`n5/5 Build an explicitly unsigned Nostr event template"
